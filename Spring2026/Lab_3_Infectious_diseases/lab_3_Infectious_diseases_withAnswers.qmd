...
If you received an error while uploading the coronavirus dataset, use the next code. If not, please skip this lines.

```{r}
#| eval: false

githubURL <- "https://github.com/jesusNPL/BiodiversityScience/raw/master/Spring2026/Lab_3_Infectious_diseases/data/coronavirus.RData"
...
coronavirus %>%
  filter(type == "confirmed") %>% # filter confirmed cases
  group_by(country) %>% # group cases by country
  summarise(total = sum(cases)) %>% # sum all cases
  arrange(-total) %>% # select the countries with more cases
  head(20) %>% 
...
**Answer:**
Below is the figure for the 20 countries with the least reported cases. Of these, the Winter Olympics 2002 and Summer Olympics 2020 are also listed. The Olympics were included in the list because the hosting cities received people from across the world.

```{r}

# Get countries with less confirmed cases by country
coronavirus %>%
  filter(type == "confirmed") %>%
  group_by(country) %>%
...
# Plot countries with less confirmed cases by country
coronavirus %>%
  filter(type == "confirmed") %>% 
  group_by(country) %>%
  reframe(total = sum(cases)) %>% 
...
We can also omit the Olympics and plot the results for actual countries.
...
coronavirus %>%
  ...
  summarise(total = sum(cases)) %>% 
  arrange(desc(total)) %>% 
  filter(country != "Summer Olympics 2020") %>% 
...
coronavirus %>%
  ...
  scale_x_continuous(labels = scales::label_number(suffix = " M", 
                                                   scale = 1e-6)) + 
  labs(x = "Number of cases", 
       y = "Countries with more reported cases | Olympics not included") + 
  theme_biosci()
...
**Answer:**
The top five countries/territories with less reported cases were Kiribati, Tuvalu, Holy See, Antarctica, and North Korea. Note that [Diamond Princess](https://www.princess.com/en-us/ships-and-experience/ships/di-diamond-princess) and [MS Zaandam](https://www.hollandamerica.com/en/us/cruise-ships/zaandam/7) are cruise ships.
...
The countries with more recovery cases were India, Brazil, Russia, Turkey and Argentina.

```{r}
## Get countries with more recovery cases
coronavirus %>%
...
Plot the top 20 with more reported cases of recovery.
...
Conversely the countries with less reported cases of recovery were Tanzania, Grenada, Holy See, Solomon Islands
...
**Answer:**
The data available is very variable and may not be completely accurate. As such, we should take care in interpreting this information.
...
If we look at the US coronavirus data it contains information of the number of recoveries (**R**), the number of new cases (**I**) and the deaths. Let's isolate those data and inspect if there are some trends.
...
deaths_us <- coronavirus %>% 
  filter(country == "US") %>% 
  arrange(desc(date)) %>% 
  filter(type == "death")

recovered_us <- coronavirus %>% 
  filter(country == "US") %>% 
  arrange(desc(date)) %>% 
  filter(type == "recovery")
...
**Answer:**
These results are interesting as we can see infectious disease dynamics in the United States.

*Please explain the trend temporally; in other words, do the spikes in the number of cases match the US holidays?*

**Answer:**
By looking at the trend, yes, the peaks match with the US national holidays or vacation periods. Interestingly, the highest peak of reported cases was between the end of December 2021 and mid-January 2022.

*What about the number of deaths and the number of recovered people?* 

**Answer:**
By plotting the number of recovery cases over time, we can see a continuous increase between March 2020 and December 2020. However, since January 2021, the reported recovery cases have been zero.
...
# Total population (estimated) for the State of Minnesota by 2025
N <- 5830405

```

By observing the trend in the number of cases for MN, we can see that the first date with records of infected subject was in **March 06, 2020** (you can double check to confirm that date), so, let's set that day as a starting point for our $SIR$ model. 
...
Moreover, as more than three years have passed since the pandemic started, the variability in the number of cases is huge, so let's select the first ~60 days (two months) of cases to fit our model and then use the fitted model to predict the dynamics of COVID-19 for the entire year.
...
**Answer:**
Despite its simplicity, the SIR model can still be used to explore the dynamics of infectious diseases. However, we must exercise great care if we want to use its output to make predictions.

*Do you see any other interested pattern? If so, please explain*

**Answer:**
It seems that under the SIR model, the number of infected subjects increases linearly. However, the actual data shows that the number of reported infected cases started to saturate after 20 days.
...
::: callout-tip
## On the derivation of the back-of-the-envelope $R_0$ 
...
To fit our model we used data within two months since the first reported case, now let's explore what would it happen if no public intervention (i.e., quarantine) was applied. Here, using the fitted model we will extrapolate up to 365 days.

```{r}
#| eval: true

times <- 1:365 # time in days

fit_365 <- data.frame(ode( 
  y = init, times = times, 
  func = SIR, parms = Opt_par))
...
To better explore the data, let's make some figures that allow us to see what would happened the first 365 days of the pandemic in the state of Minnesota in a hypothetical case of no intervention.
...
matplot(fit_365["time"], fit_365[, 2:4], 
        type = "l", 
        xlab = "Days", 
...
Let's try plotting the residuals

```{r}
#| eval: true

fit_incidence$infected <- Infections$new_case

fit_incidence %>% 
  filter(infected > 0) %>% 
  mutate(diff = log(infected) - log(I)) %>% 
  ggplot(aes(x = log(I), log(infected), color = diff)) + 
  geom_point(size = 3) + 
  geom_abline(intercept = 0, slope = 1, linetype = 2, linewidth = 2, color = "gray") + 
  lims(x = c(0, 7), y = c(0, 7)) + 
  labs(x = "Modeled Incidences", y = "Observed Incidences") + 
  scale_color_viridis_c(option = "B", name = "Residuals") + 
  theme_biosci() 
...
**Answer:**
SIR model projections are visually appealing but can't recover the actual data accurately. That is, some days show overprediction and others underprediction.

*Please explain the trend of each compartment in a temporal way?*

**Answer:**
Despite the SIR models not being accurate enough to predict the dynamics of infectious diseases, we can still use the model projections to have a scenario in which no intervention (e.g., quarantine) is applied. Under this scenario of no intervention, the number of infected subjects increased linearly until day 123 (I = 118780) after the beginning of the pandemic. Then, there is a continuous decline in the number of infected subjects. The number of recovered/death cases also increases linearly until day 280 (R = 2114899). Then, the number of R subjects continues increasing but at lower numbers, which gives an impression of saturation. Finally, the number of susceptible (S) cases has been steadily declining but slowly declining over time. It has changed drastically as the number of R subjects has increased. 
...
# Peak of the pandemic for the first three month
...
The challenge for this assignment is to repeat the process by selecting another state in the United States and discuss if the $R_0$ obtained differs from that obtained for the state of Minnesota. You can speculate on the causes of the differences.
