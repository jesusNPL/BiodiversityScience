...
packages <- c("terra", "geodata", "sp", "spThin", "sf", "maps", "dismo", 
              ...
              "Metrics", "ROCR", "ggraph", "plotly", "sdmvspecies")
...
A message will pop out in your console "Additional software needs to be downloaded and installed for torch to work correctly" and "Do you want to continue? (Yes/no/cancel)" please type "y" and hit return.
...
biosNames <- list.files("data/climate/wc2.1_5m/", 
                        pattern = "tif$")

# Please double check your folder path. 

bios <- rast(paste0("data/climate/wc2.1_5m/", # folder directory 
...
#US_poly <- read_sf("data/states.shp")
...
source("functions/select07_mod.R")
...
Let's save this final raster file in your hard drive.
...
# rescale predictions to vary between 0 to 1
quemin_ENM <- sdmvspecies::rescale(stack(quemin_ENM))
...
# rescale predictions to vary between 0 to 1
quemin_ENM_uncertainty <- sdmvspecies::rescale(stack(quemin_ENM_uncertainty))
...
bios_future <- geodata::cmip6_world(model = "CNRM-CM6-1", 
                           ssp = "585", # socio-economic pathway
                           time = "2061-2080", 
                           var = "bioc", 
                           res = 5, 
                           path = "data/")
...
Mask future environmental conditions to the extent of Quercus minima and select the variables relevant for predictions.

```{r}
#| eval: true

bios_quemin_future <- mask(crop(bios_future, ext(bios_quemin_sel)), ext(bios_quemin_sel))

bios_quemin_future_sel <- bios_quemin_future[[c("bio18", "bio06", "bio09", "bio05", "bio15", "bio12", "bio02")]]
# variables must have the same names
names(bios_quemin_future_sel) <- preds

```

Now the predictions to the future

```{r}
#| eval: true

## Scale the predictors 
bios_quemin_future_sel_scale <- terra::scale(bios_quemin_future_sel)

## Predict the Grinnellian niche for Quercus minima
quemin_ENM_future <- terra::predict(object = bios_quemin_future_sel_scale,
                  model = quemin_DNN,
                  fun = customPredictFun, 
                  na.rm = TRUE) 
...
quemin_ENM_future <- sdmvspecies::rescale(stack(quemin_ENM_future))
