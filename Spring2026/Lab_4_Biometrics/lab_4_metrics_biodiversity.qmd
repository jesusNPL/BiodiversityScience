...
load("data/NEON/RawData_NEON_lab4.RData")
...
head(unique(sel$namedLocation), 5) 
tail(unique(sel$namedLocation), 5)

head(unique(sel$endDate), 5)
tail(unique(sel$endDate), 5)
...
Select the site __HARV__ and the year __2024__.

```{r}
#| eval: true

HARV <- sel %>% 
  filter(siteID == "HARV" & Year == 2024)
...
We will use the **Plants of the World** source through the **[Taxonomic Name Resolution Service (TNRS)](https://tnrs.biendata.org)** tool. 
...
HARV_data <- inner_join(x = HARV, y = taxonomy, 
...
Let's plot the association between SR and PD, but now using SES-PD...
...
# plot
HARV_CDM %>% 
  ggplot(aes(x = ntaxa, y = pd.obs.z)) + 
  geom_point(size = 3, color = "darkgray") + 
  labs(x = "Species richness", y = "SES-PD") + 
  theme_biosci()
...
# Scheiner 2012 qD(p)
#source("https://raw.githubusercontent.com/jesusNPL/BiodiversityScience/master/Spring2024/R-Functions/qDp.R")
...
HARV_CDM <- HARV_CDM %>% 
  mutate(qDP = qDp(tree = matched$phy, 
                   comm = matched$comm, 
                   q = 2))
...
HARV_mds_scores %>% 
  ...
  #geom_jitter(size = 5, height = 0.25) + 
  scale_color_viridis_d(option = "A") +
  theme_biosci()

```

Seems that MPD-obs and RaoD are overlapped. Let's plot their association...

```{r}
#| eval: true
#| 
HARV_CDM %>% 
  ggplot(aes(x = RaoD, y = mpd.obs)) + 
  geom_point(size = 3, color = "darkgray") + 
  labs(x = "RaoD", y = "MPD") + 
  theme_biosci()
...
HARV_CDM %$% 
  cor.test(RaoD, mpd.obs, use = "complete.obs")

```

*What do you think?* 

*Is there a metric that answer all our interests/questions?*

*Which metric would you use for biodiversity assessments?*
...
Another option is to calculate the phylogenetic diversity metrics for each year on a NEON site. For example, you can use the data from HARV but instead of doing the analysis only for the 2024 (as we did here) you can repeat the process for all years from 2013 to 2024 and discuss if the taxonomic (species richness) and phylogenetic diversities changed.
