...
coords_HARV_001 <- c(east[1], north[1]) # select plot 1
...
You can download the spectral data directly from [HERE](https://drive.google.com/file/d/14JSVK0NKm8OpKBDkX0gGIGwSggm6UOUu/view?usp=share_link). Once you download the **zip** file, copy it into the folder **data/NEON**.
...
fl <- "data/NEON/DP3.30006.001/neon-aop-products/2019/FullSite/D01/2019_HARV_6/L3/Spectrometer/Reflectance/NEON_D01_HARV_DP3_725000_4700000_reflectance.h5"
...
wlInfo <- h5readAttributes(fl, "/HARV/Reflectance/Metadata/Spectral_Data/Wavelength")
...
WL <- h5read(fl, "/HARV/Reflectance/Metadata/Spectral_Data/Wavelength")
...
reflInfo <- h5readAttributes(fl, "/HARV/Reflectance/Reflectance_Data")
...
b34 <- h5read(fl, "/HARV/Reflectance/Reflectance_Data", 
...
myEPSG <- h5read(fl, "/HARV/Reflectance/Metadata/Coordinate_System/EPSG Code")
...
rgb_harv <- lapply(rgb, # band numbers
                   ...
                   file = fl, # file from where data is extracted
...
aPixel <- h5read(fl, "/HARV/Reflectance/Reflectance_Data", 
...
if ( ! ("geoNEON" %in% installed.packages())) {
...
if ( ! ("neonhs" %in% installed.packages())) { 
  remotes::install_github('earthlab/neonhs') 
  }
...
reflMetadata <- h5readAttributes(fl, "/HARV/Reflectance" )
...
resHARV_001_df_long_mask %>% 
  ggplot() + 
  geom_line(aes(x = WL, y = Reflectance, color = scientificName, group = scientificName)) +
  scale_color_viridis_d(option = "A", 
                        labels = c("Acer rubrum", 
                                   "Betula lenta", 
                                   "Pinus strobus", 
...
This was a very long lab, and it will take some time to digest all the information. Thus, the challenge for this lab is simple. Prepare a document with all the figures generated in this tutorial and explain the figures related to the spectral profiles based on what you have learned in the lectures. 
