...
*Can we compare the coefficients of this model?*

**Answer:**
The answer might seems trivial but actually to make inferences using statistical models we first need to scale the predictors, so model coefficients are in the same scale.
...
fdata_scaled[, c(4, 6, 7)] <- apply(X = fdata_scaled[, c(4, 6, 7)], MARGIN = 2, FUN = arm::rescale)

head(fdata_scaled)

```

Now, let's us run our model with the rescale data.

```{r}
#| eval: true

fols3 <- lm(SR ~ MAT + MAP + MRD, data = fdata_scaled) 
...
autocor_SR <- ncf::correlog(fdata_scaled$x, # longitude
                            fdata_scaled$y, # latitude
                            z = fdata_scaled$SR, # species richness
                            na.rm = TRUE, 
                            increment = 1, 
                            resamp = 1)
...
autocor_ols_res <- ncf::correlog(x = fdata_scaled$x, 
                            y = fdata_scaled$y, 
                            z = residuals(fols3), 
                            increment = 1, 
                            resamp = 1)
...
sar_nb1.5.w <- spatialreg::errorsarlm(fols3, 
                                      listw = nb1.5.w, 
                                      data = fdata_scaled,
                                      quiet = FALSE, 
                                      zero.policy = TRUE, 
...
autocor_sar_nb1.5.w <- ncf::correlog(x = fdata_scaled$x, 
                                     y = fdata_scaled$y, 
...
fdata_scaled %>% 
  ...
  labs(x = "Mean annual temperature", 
       y = "Species richness") + 
  geom_abline(intercept = coef(fols3)[1], # coefficient OLS
              slope = coef(fols3)[2], 
              color = "blue", 
              linewidth = 2) + 
...
1. First option -- repeat the process for the entire lineage but instead of mapping Mean root distance, you can map other variables within the dataset, e.g., body mass, age, range size, and answer the questions.

2.  Second option -- repeat the process but using a subclade within the Infraorder Furnariides. Remember, the Infraorder Furnariides is composed of six families ("Conopophagidae", "Dendrocolaptidae" "Formicariidae", "Furnariidae", "Rhinocryptidae", "Thamnophilidae"), you just need to select a family within Furnariides and explain the differences (if any) with the results found today.
