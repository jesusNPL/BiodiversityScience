...
furnariidaeData <- furnaData %>% 
  filter(Family3 == "Furnariidae") #%>% 
  #dplyr::select(Species3, `Hand-Wing.Index`, Range.Size) 
...
rownames(furnariidaeData) <- furnariidaeData$species

traitNames <- names(furnariidaeData)[c(17, 36)]
...
compData <- caper::comparative.data(
  ...
  data = furnariidaeData %>% 
    dplyr::select(`Hand-Wing.Index`, Range.Size, species),
  names.col = "species",
  vcv.dim = TRUE,
  warn.dropped = FALSE
)

mod <- pgls(log(`Hand-Wing.Index`) ~ log(Range.Size), lambda = "ML", data = compData) 
