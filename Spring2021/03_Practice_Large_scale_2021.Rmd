...
You can download the data directly on your computer by clicking [**Occurrences**](https://www.dropbox.com/s/rpql1s1pv7d0plz/live.oaks.txt?dl=0),  [**Geographical Ranges**](https://www.dropbox.com/s/vf7cbamzc8pyfz5/Ranges.zip?dl=0), [**Phylogenetic tree**](https://www.dropbox.com/s/lmjvnuo1sogszqp/furnariiMCC.nex?dl=0) and [**Environmental predictors**](https://www.dropbox.com/s/wjyq4o64dtfln4u/Environmental.zip?dl=0) and store them in the folder named **Data"**.
...
```{r, eval=FALSE}
packages <- c("maptools", "rgdal", "raster", "rangeBuilder", "spdep", "ncf", "geiger", 
              "letsR", "rworldmap", "spatialreg", "picante", "ape") 
```
...
Now plot the occurrences of *Quercus virginiana*
...
To create our first species geographical range let's use the **{rangeBuilder}**. This package create a polygon based on the spatial distribution of the coordinates.
...
Now plot the output and overlap the occurrences.
...
To load the Furnariides geographical ranges we will use the function **readOGR** from the package **{rgdal}**.
...
*Explain the distribution for both species (i.e., Furnarius rufus [green polygon] and Anabazenops dorsalis [red polygon])*
...
This paragraph was extracted entirely from **Dormann et al. (2007)**:
...
*What is telling us this OLS models?*
...
Let's use an correlogram to explore the spatial autocorrelation. Remember, spatial autocorrelation (it can also be temporal) is a measure of similarity (**correlation**) between nearby observations. Thus, high values means high spatial autocorrelation.
...
# this will take a while, ~30 seconds in Jesús's computer
```
...
Pinto-Ledezma, J. N., Simon, L. M., Diniz-Filho, J. A. F., & Villalobos, F. (2017). The geographical diversification of Furnariides: the role of forest versus open habitats in driving species richness gradients. Journal of Biogeography, 44(8), 1683–1693. doi:10.1111/jbi.12939
