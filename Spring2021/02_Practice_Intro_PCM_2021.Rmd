...
You can download the data directly on your computer by clicking [**HERE**](https://www.dropbox.com/s/mdfxwaak7dmvx89/anolis.phy?dl=0) and [**HERE**](https://www.dropbox.com/s/s0y4g5efavgewgf/anolisDataAppended.csv?dl=0) and store them in a folder named **Data"**.
...
```{r, eval = FALSE}
pglsModel <- gls(awe ~ host, correlation = corBrownian(phy = anoleTree, form=~names(awe)), method = "ML")
```
