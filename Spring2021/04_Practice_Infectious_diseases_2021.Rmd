...
We can see that there is trend in the number of confirmed Infected subjects since the first case registered in the US, happily, the number of reported cases are declining steadily.
...
plot(1:nrow(covid_mn), covid_mn$tot_cases, type = "b", 
     ylab = "Infected", xlab = "Days since the pandemic started")
```
...
More specifically, in order to solve the equations what we need is to minimize the sum of the squared differences ($RSS$) between the number of $I$ subjects (infected) and time $t$--$I(t)$--and the number of predicted cases by our model, i.e., $\hat{I}(t)$:
...
By observing the trend in the number of cases for MN, we can see that the first record of infected subject was in March 06 2020, so, let's set that day as a starting point for our $SIR$ model. Moreover, as more than a year has passed since the pandemic started, the variability in the number of cases in very huge, so let's select the first 30 days of cases to fit our model and then using the fitted model to predict for the next couple of months.
...
Infected <- subset(covid_mn, Date2 >= ymd(start_date) & Date2 <= ymd(end_date))$tot_cases
...
As we now know that the number of $I$ were increasing during the first days of the pandemic, let's model de dynamic of COVID-19 during that period of time.
...
Now using the fitted $\beta$ and $\gamma$ values, we can see if our model recovers the observed trend in the US.
...
Let's plot the estimated $I$ over the 30 days. We will use a kind of plot named **semi-log plot**, this plot allow us the read the difference between the expected (fitted) and observed number of confirmed cases over time.
...
Hmmm, looks like the fitted model (despite it converged well) does not accurately recover the number of infected cases in the Minnesota, in fact, it tend to underestimate the number of cases, although we can see an steady increment on the number of cases since the first detected case. 
...
The estimated $R_0$ is 1.549 which suggests that for every sick subject 1.5 subjects can get infected by COVID-19. This value is lower when compared to other diseases and even lower than the estimated $R_0$ for COVID-19 in the literature that ranges between 2.5 and 3. One potential explanation for this low $R_0$ can be due the fact the first 30 days, the number of observed cases were very low when compared with the large population size ($N$) in the state of Minnesota.

## Evaluating the outbreak under no intervention

To fit our model we used the first 30 days since the first reported case, now let's explore what would happen if no public intervention (i.e., quarantine) was applied. Here, using the fitted model we will extrapolate up to 90 days
...
To better explore the data, let's make some figures that allow us to see what happened the first 90 days of the pandemic in the state of Minnesota.
...
Bjørnstad, O. N. (2018). Epidemics. Use R! doi:10.1007/978-3-319-97487-3
