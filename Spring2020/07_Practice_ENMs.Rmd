...
```{r, warnings = FALSE, message = FALSE, eval = FALSE}
# Variable selection (execute a factorial analysis [FA])
...
Based on the results of the FA and the specialist knowledge (Jesús ;p) we can select the next variables: bio1, bio2, bio15, bio4, bio2.
...
```{r, warnings = FALSE, message = FALSE, eval = FALSE}
id.back <- sample(1:nrow(climSA0k), 130) # nrow(hyacinth_var) retunr the same number of TRUE occurrences
length(id.back)
```
```{r, warnings = FALSE, message = FALSE, eval = FALSE}
