...
shinyServer(function(input, output) {
  ...
  x <- reactive({
    ...
    replicate(input$NS, cumsum(rnorm(n = input$MY, sd = input$rate)))
  })
  ...
  output$distPlot <- renderPlot({  
    ...
    abline(v = input$dens, lwd = 3, col = "red")
    # now we plot the distribution switched from a histrogram this looks prettier IMHO
    plot(density(x()[input$dens,]), 
         xlim = c(min(x()), max(x())),
         col = 'black', 
         xlab = "Trait value",
         ylab = "",
         lwd = 3,
         main = paste("Trait distribution at generation", input$dens))
